...
1. Read `data/quotes.json` and `data/flows.json`. If either does not exist or
   `fetched_at` is more than 24 hours old, stop and prompt the user to run
   `stocklux refresh`, then wait before continuing.
2. Read `data/watchlist.json` to find the thesis this ticker is attached to,
   ...
   (see the methodology's position-context rule).
...
- **Incremental updates must be self-contained.** The latest memo is what
  ...
  already in motion. When doing an incremental update, the Delta section
...
- `flows`: first cite the hard numbers from flows.json (short interest as %
  ...
  the price/volume accumulation_hint), then search online for recent 13F
...
- `sentiment`: search X/Reddit/financial media for discussion density and
  direction around this ticker and its theme; judge crowding — "does everyone
  already know this story" — and whether retail narrative and institutional
  narrative have diverged.
...
- Good-buy range = scenario EPS × a reasonable multiple (selected from the
  methodology's multiple table, with justification stated).
- **Bear/base/bull price targets (price_targets, 12-month)**, each with its
  ...
  - bull = high-scenario EPS (thesis fully plays out) × high end of the
    multiple range
  Price targets are **[INFERENCE]**, carry a confidence level; the body must
  include a small bear/base/bull comparison table (EPS assumption / multiple /
...
it discriminates. Search online for the actual dates — do not guess them.
This section gives `wait_for_pullback` and `watch_only` a clock.

### 6. Risks & thesis-killer
...
- New `review_trigger`: written as a specific, observable event. Fold in the
  deciding observables from any open divergences (step 7) so disagreements
  get re-tested instead of forgotten.

### 7. Divergence (only when a user view conflicts with a ruling)
...
- The ten-state verdict + confidence level, derived by walking the
  ...
  confidence-propagation cap (overall confidence ≤ valuation confidence and
  ≤ payload-dimension confidence).
...
- Finish by reporting back to the user out loud: the ten-state verdict, the
  good-buy range vs. current price, the single most important risk — plus,
