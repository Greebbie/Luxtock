...
7. **Price figures must always be quoted from `data/quotes.json`** (to stay
   ...
   `fetched_at` is more than 24 hours old, prompt the user to run
   `stocklux refresh` before continuing. The only exception: candidates
...
6. Otherwise (thesis intact/weakening, `verdict` field not `above_range`):
   - `verdict: below_range` + `chain`/`fundamentals` favorable → not
     holding: `enter`; holding: `hold` (adding is a sizing decision the
     framework doesn't make).
...
dimension carrying the thesis's payload assumption. A high-conviction
verdict built on a low-confidence payload is a contract violation.
...
3. **Scenario modeling & valuation** — pass-through math, scenario EPS, the
   good-buy derivation, and the bear/base/bull derivation (each of bear/base/bull
   must show its EPS assumption × multiple assumption explicitly; price targets
   are **[INFERENCE]** and carry a confidence level — never just pick a round
   number)
...
5. **Catalysts (next 1–2 quarters)** — 2–5 dated, observable events (earnings
   ...
   date/window, what to watch, and which scenario tier (bear/base/bull) it
   discriminates. This is what gives time-flavored verdicts
   (`wait_for_pullback`, `watch_only`) an actual clock; `review_trigger`
   says when to re-analyze, catalysts say when the story gets tested.
6. **Risks & thesis-killer**
7. **Divergence** — required whenever a user view conflicts with a dimension
   ...
   no conflict.
8. **Summary & rulings** — signal table roll-up, the ten-state verdict
